...
DNNToTest = {'CIFAR100-DENSENET'}; %try to only use one DNN each time, otherwise, it will be very complecate
%=============================================================
...
% Ensemble_Learning_Method_Prediction_DIR ='ccp8-2.5-250000';

Ensemble_Learning_Method_Prediction_DIR ='ip1-2.5-100000iters';
% Ensemble_Learning_Method_Name ='RI'; %not useful any more
...
%     totoal_num_phase = DNN_MODELS{DNN_ID}.TOTAL_NUM_PHASE;
    ...
    %DNN_MODELS{DNN_ID}.MAX_NUM_ITERATION
    iterations = [20000:20000:100000];
%     iterations = [50000:25000:75000];%50000:15000:65000
%     iterations = 280000;
    for iter_id = 1 : length(iterations )
    % =======================================
        iter = iterations(iter_id);
...
%             iter = iterations(k);
            ...
            %===================================================================
            ResultingFile.experiment_flag =  'TEST';  %note the results are saved in BASELINE
            %===================================================================


            ResultingFile.totoal_num_phase =  totoal_num_phase; %specified by myself,
            ResultingFile.save_file_prefix = [Ensemble_Learning_Method_Prediction_DIR '_RETRAIN' num2str(retrain_id) '_ITERATION_' num2str(iter) ];
