function [accu_ensemble, accu_oracle,final_pred] = MakeEnsembleOriginal100RunWeighted_A( pred_phases, weights,ground_truth, detail_flag)
...
%     random_order = 1: num_phase; %only for debug use
    ...
    % for k == 1, we can not use the minimum_hamming_dist, because the minimum dist 
     ...
     [final_pred, ensemble_result(:,:,k) ] = WeightedMajorityVoting_A( preds , weights, orderd_class_labels);
%     for i = 1 : num_image
